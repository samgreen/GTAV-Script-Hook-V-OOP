...
bool featureVehIndicatorRight			=   false;
bool featureVehIndicatorLeft			=	false;
bool featureVehIndicatorLeftUpdated		=	false;
bool featureVehIndicatorRightUpdated	=	false;

bool featureVehEngine					=	false;
bool featureVehEngineUpdated			=	false;

bool featureVehSearchLight				=	false;
bool featureVehSearchLightUpdated		=	false;
...
void update_features() 
{
	...
	if (featureVehSearchLight) {
		if (bPlayerExists && PED::IS_PED_IN_ANY_VEHICLE(playerPed, 0)) {
			Vehicle veh = PED::GET_VEHICLE_PED_IS_USING(playerPed);
			if (VEHICLE::IS_VEHICLE_SEARCHLIGHT_ON(veh)) {
				VEHICLE::SET_VEHICLE_SEARCHLIGHT(veh, 1, 0);
			}
		}
	}

	// Search Light (Police)
	if (featureVehSearchLightUpdated) {
		if (bPlayerExists && !featureVehSearchLight && PED::IS_PED_IN_ANY_VEHICLE(playerPed, 0)) {
			Vehicle veh = PED::GET_VEHICLE_PED_IS_USING(playerPed);
			if (VEHICLE::IS_VEHICLE_SEARCHLIGHT_ON(veh)) {
				VEHICLE::SET_VEHICLE_SEARCHLIGHT(veh, 0, 0);
			}
		}
		featureVehSearchLightUpdated = false;
	}

	// Left Indicator
	if (featureVehIndicatorLeftUpdated) {
		if (bPlayerExists && !featureVehIndicatorLeft && PED::IS_PED_IN_ANY_VEHICLE(playerPed, 0)) {
			Vehicle veh = PED::GET_VEHICLE_PED_IS_USING(playerPed);
			VEHICLE::SET_VEHICLE_INDICATOR_LIGHTS(veh, 1, 0);
		}
		featureVehIndicatorLeftUpdated = false;
	}

	// Right Indicator
	if (featureVehIndicatorRightUpdated) {
		if (bPlayerExists && !featureVehIndicatorRight && PED::IS_PED_IN_ANY_VEHICLE(playerPed, 0)) {
			Vehicle veh = PED::GET_VEHICLE_PED_IS_USING(playerPed);
			VEHICLE::SET_VEHICLE_INDICATOR_LIGHTS(veh, 0, 0);
		}
		featureVehIndicatorRightUpdated = false;
	}

	if (featureVehIndicatorLeft)
	{
		if (bPlayerExists && PED::IS_PED_IN_ANY_VEHICLE(playerPed, 0)) {
			Vehicle veh = PED::GET_VEHICLE_PED_IS_USING(playerPed);
...
void process_veh_menu()
{
	...
	static struct {
		...
		// Indicator (by Nac)
		{"RIGHT INDICATOR LIGHT", &featureVehIndicatorRight, &featureVehIndicatorRightUpdated},
		{"LEFT INDICATOR LIGHT", &featureVehIndicatorLeft, &featureVehIndicatorLeftUpdated},
		{"USE SEARCH LIGHT", &featureVehSearchLight, &featureVehSearchLightUpdated},
		{ "SET LICENSE PLATE TO AXEL", NULL, NULL }
	...
	while (true)
	{
		...
		if (bSelect)
		{
			...
			// common variables
			BOOL bPlayerExists = EntityObj::exists(PlayerObj::getId().getHandle());
			PlayerObj player = PlayerObj(PlayerObj::getId());
			PedObj playerPed = PedObj(PLAYER::PLAYER_PED_ID());
			...
			switch (activeLineIndexVeh)
			{
				...
				case 1:
					if (bPlayerExists) 
					{
						if (playerPed.isInsideVehicle())
						{
							VehicleObj veh = playerPed.getVehicleUsing();
							veh.setPrimaryColor(rand() % 255, rand() % 255, rand() % 255);
							if (veh.hasSecondaryColor())
								veh.setSecondaryColor(rand() % 255, rand() % 255, rand() % 255);
						} else
						{
				...
				case 2:
					if (bPlayerExists) 
						if (playerPed.isInsideVehicle())
							playerPed.getVehicleUsing().setFixed();
						else
							set_status_text("Player isn't in a vehicle!");
					break;
				case 9:
					if (bPlayerExists)
						if (playerPed.isInsideVehicle()) {
							VehicleObj bVehicle = playerPed.getVehicleUsing();
							bVehicle.setLicensePlateText("EXAMPLE");
						}				
				// switchable features
				default:
...
void reset_globals()
{
	...
	featureVehIndicatorLeft			=
	featureVehIndicatorRight		=
